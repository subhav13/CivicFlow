...
| 1:48–2:08 (20s) | “The optional Gemini companion uses that same current surface. A mutation pauses for a visible button decision, not a spoken approval.”           | On the current Site if the prior gate is available, propose a synthetic update by text or voice; show the confirmation card, then click **Save change**.                                                 | `hosted-tested` / `user-confirmed` for the accepted v10 assistant behavior; current v11 publication is documentation-only; `local-tested` for confirmation policy.                          | Use the local text/manual WebMCP path; state that voice is optional and not revalidated in this recording. |
...
- Confirm the runtime and any shown tool calls match one exact source/deployment
  ...
  documentation-only Sites v11 publication.
