...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| 0:00–0:12 (12s) | “CivicFlow is a synthetic research demo for people and agents working together on a structured application workflow.”                             | Open the accepted CivicFlow Site and show the six-section portal.                                                                                                                                        | `hosted-tested` / `user-confirmed`: Sites v10, source SHA `0632c5d503a98b9d37e2450f2e9c1f3265698930`.                       | Use a local build and label it `local-tested`; do not imply it is the hosted version.                      |
...
| 1:48–2:08 (20s) | “The optional Gemini companion uses that same current surface. A mutation pauses for a visible button decision, not a spoken approval.”           | On the accepted Site if the prior gate is available, propose a synthetic update by text or voice; show the confirmation card, then click **Save change**.                                                | `hosted-tested` / `user-confirmed` for the previously accepted v10 assistant state; `local-tested` for confirmation policy. | Use the local text/manual WebMCP path; state that voice is optional and not revalidated in this recording. |
...
- Confirm the runtime and any shown tool calls match one exact source/deployment
  identity. Do not mix a later local documentation candidate into a claim that
  belongs to Sites version 10.
